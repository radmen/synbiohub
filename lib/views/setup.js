...
function setupPost (req, res, settings) {
  ...
  if (settings.uriPrefix[settings.uriPrefix.length - 1] !== '/') {
    settings.uriPrefix += '/'
  }
  ...
  config.set('webOfRegistries', { [settings.uriPrefix.slice(0, -1)]: settings.instanceUrl.slice(0, -1) })
  ...
  config.set('databasePrefix', settings.uriPrefix)

  config.set('triplestore', extend(config.get('triplestore'), {
    graphPrefix: settings.uriPrefix,
    defaultGraph: settings.uriPrefix + 'public',
