...
module.exports = function(req, res) {
    ...
    fetchSBOLObjectRecursive(uri + '/implementation', graphUri).then((result) => { //TODO FIX URL
	...
	if (meta.description != '') {
        ...
        meta.attachments = attachments.getAttachmentsFromTopLevel(sbol, genericTopLevel,
				...
				//TODO ASK ABOUT THIS IT'S A HACK FOR NOW
        locals.canEdit = true
	...
	locals.share = share
        ...
        locals.submissionCitations = submissionCitations
            ...
            return citation.citation
				...
				fetchSBOLObjectRecursive(uri + '/testact', graphUri).then((result) => {
						asc = result.object._sbolDocument._provAssociations[0]
						agent = asc._agent._description
						plan = asc._plan._description
						location = genericTopLevel._annotations[0]['value'].split('/').pop()

						console.log(agent)
						console.log(plan)
						console.log(locals.meta.description)
						//
						// console.log(asc)
				...
				}).then(res.send(pug.renderFile('templates/views/implementation.jade', locals)))
