...
module.exports = function(req, res) {
    ...
    fetchSBOLObjectRecursive(uri, graphUri).then((result) => {
        ...
        if(!agent || agent instanceof URI) {
            locals = {
                config: config.get(),
                section: 'errors',
                user: req.user,
                errors: [ uri + ' Record Not Found: ' + agent ]
            }
            res.send(pug.renderFile('templates/views/errors/errors.jade', locals))
            return Promise.reject()
        }
        meta = sbolmeta.summarizeGenericTopLevel(agent)
        if(!meta) {
            locals = {
                config: config.get(),
                section: 'errors',
	...
	if (meta.description != '') {
        ...
        meta.attachments = attachments.getAttachmentsFromTopLevel(sbol, agent,
        ...
        if(!remote && req.user) {
            ...
            const ownedBy = agent.getAnnotations('http://wiki.synbiohub.org/wiki/Terms/synbiohub#ownedBy')
	...
	if (agent.wasGeneratedBy) {
	    meta.wasGeneratedBy = { uri: agent.wasGeneratedBy.uri?agent.wasGeneratedBy.uri:agent.wasGeneratedBy,
				    url: uriToUrl(agent.wasGeneratedBy,req)
				  }
	}
	...
	locals.rdfType = {
	    name : 'Agent',
	    url : 'http://wiki.synbiohub.org/wiki/Terms/SynBioHub#Agent'
	}

        locals.annotations = filterAnnotations(req,agent.annotations);
	...
	locals.citationsSource = citations.map(function(citation) {
        ...
        res.send(pug.renderFile('templates/views/agent.jade', locals))
